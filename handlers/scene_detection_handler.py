...
class SceneDetectionHandler(BaseHandler):
    ...
    def _get_stats_path(self, video_path: str) -> str:
        """Формирует путь к stats CSV."""
        if self.stats_csv_dir:
            out_dir = Path(self.stats_csv_dir)
        else:
            video_name = Path(video_path).stem
            out_dir = Path("temp")
            out_dir.mkdir(exist_ok=True)

        return str(out_dir / f"{Path(video_path).stem}_scene_stats.csv")
